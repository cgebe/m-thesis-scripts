...
open( my $fulltextFile, ">:encoding(utf8)", "gcd.facts" )
  || die "Problems opening file gcd.fulltexts: $!\n";
open( my $typeFile, ">:encoding(utf8)", "gcd.courts" )
  || die "Problems opening file gcd.types: $!\n";
open( my $fulltextTestFile, ">:encoding(utf8)", "gcd.facts-test" )
|| die "Problems opening file gcd.fulltexts: $!\n";
open( my $typeTestFile, ">:encoding(utf8)", "gcd.courts-test" )
|| die "Problems opening file gcd.types: $!\n";
        ...
        if ( $document =~ /xml$/ ) {
            ...
            if ( !( $text eq '' || $text =~ /^ *$/ ) ) {
                if (int(rand(100)) <= 1) {
                    ...
                    print $typeTestFile $court . "\n";
                } else {
                    print $fulltextFile $text . "\n";
                    print $typeFile $court . "\n";
                }
            }
            close $F;
        }
