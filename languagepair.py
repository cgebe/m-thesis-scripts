...
dcepLanguages = ["BG", "CS", "DA", "DE", "EL", "EN", "ES", "ET", "FI", "FR", "GA",
    "HU", "IT", "LT", "LV", "MT", "NL", "PL", "PT", "RO", "SK", "SL", "SV", "TR"]
...
def main():
    ...
    defaultDelimiter = " ~~~ "
    parser.add_option("--no-merge", action="store_true", dest="noMerge",
                      help="Keep the output bidocuments in separate files under bitext/L1-L2/, instead of merging them and writing them to the standard output.")
    parser.add_option("--not-just-bisentences", action="store_false", dest="justBisen",
                      default=True, help="Save all alignment units, not just 1-to-1 correspondences.")
    parser.add_option("--delimiter", dest="delimiter", type="string", default=defaultDelimiter,
                      help="String for delimiting sentences within alignment units. Only meaningful when combined with --not-just-bisentences. Default value: '" + defaultDelimiter + "'.")
...
#	help="Agressiveness of context-based bisentence filtering. Between 0 and 100. By default it is not employed. Cannot be combined with --not-just-bisentences.")
    ...
    for line in f:
    	...
    	if os.path.isfile(ladder):
    	    lines = ladder2text.process(ladder, doc1, doc2,
            ...
            if (docid not in testDocs) :
                # training set
                resultFilename1 = "moses/dcep."+lp.lower()+"."+heuristicL1.lower()
                resultFilename2 = "moses/dcep."+lp.lower()+"."+heuristicL2.lower()
                resultFilename3 = "moses/dcep."+lp.lower()+".info"

                resultFile1 = open(resultFilename1, "a")
                resultFile2 = open(resultFilename2, "a")

                for l in lines:
                    sens = l.split('\t')
                    resultFile1.write(sens[0]+"\n")
                    resultFile2.write(sens[1]+"\n")

                resultFile1.close()
                resultFile2.close()

                resultFile3 = open(resultFilename3, "a")
                resultFile3.write(docid+"\n")
                resultFile3.close()
            else:
                # test set
                resultFilename3 = "moses/dcep."+lp.lower()+"-test."+heuristicL1.lower()
                resultFilename4 = "moses/dcep."+lp.lower()+"-test."+heuristicL2.lower()
                resultFilename5 = "moses/dcep."+lp.lower()+"-test.info"

                resultFile3 = open(resultFilename3, "a")
                resultFile4 = open(resultFilename4, "a")

                for l in lines:
                    sens = l.split('\t')
                    resultFile3.write(sens[0]+"\n")
                    resultFile4.write(sens[1]+"\n")

                resultFile3.close()
                resultFile4.close()

                resultFile5 = open(resultFilename5, "a")
                resultFile5.write(docid+"\n")
                resultFile5.close()
            '''
    		outputBytes = ""
